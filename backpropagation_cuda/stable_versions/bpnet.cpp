...
void bpnet::applyBatchCumulations(float alpha, float momentum)
{
    ...
    float *errorWeight_C;
    float *errorGain_C;
    cudaMalloc((void **)&deltavalues_C,(m_outputlayer.neuroncount)*(m_outputlayer.inputcount)*sizeof(float));
    cudaMalloc((void **)&weights_C,(m_outputlayer.neuroncount)*(m_outputlayer.inputcount)*sizeof(float));
    cudaMalloc((void **)&wgain_C,(m_outputlayer.neuroncount)*sizeof(float));
    cudaMalloc((void **)&errorWeight_C,(m_outputlayer.neuroncount)*sizeof(float));
    cudaMalloc((void **)&errorGain_C,(m_outputlayer.neuroncount)*sizeof(float));


    float *deltavalues_D=new float[m_outputlayer.neuroncount*m_outputlayer.inputcount];
    float *weights_D=new float[m_outputlayer.neuroncount*m_outputlayer.inputcount];
    float *wgain_D=new float[m_outputlayer.neuroncount];
    float *errorWeight_D=new float[m_outputlayer.neuroncount];;
    float *errorGain_D=new float[m_outputlayer.neuroncount];;
    ...
    cudaMemcpy(deltavalues_C, deltavalues_D, (m_outputlayer.neuroncount)*(m_outputlayer.inputcount)*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(weights_C, weights_D, (m_outputlayer.neuroncount)*(m_outputlayer.inputcount)*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(wgain_C, wgain_D, (m_outputlayer.neuroncount)*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(errorWeight_C, errorWeight_D, (m_outputlayer.neuroncount)*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(errorGain_C, errorGain_D, (m_outputlayer.neuroncount)*sizeof(float), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(deltavalues_D, deltavalues_C, (m_outputlayer.neuroncount)*(m_outputlayer.inputcount)*sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(weights_D, weights_C, (m_outputlayer.neuroncount)*(m_outputlayer.inputcount)*sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(wgain_D, wgain_C, (m_outputlayer.neuroncount)*sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(errorWeight_D, errorWeight_C, (m_outputlayer.neuroncount)*sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(errorGain_D, errorGain_C, (m_outputlayer.neuroncount)*sizeof(float), cudaMemcpyDeviceToHost);
    ...
    for(i=(m_hiddenlayercount - 1);i>=0;i--)
    {
        for(j=0;j<m_hiddenlayers[i]->neuroncount;j++)
        {
            ...
            {
                delta=m_hiddenlayers[i]->neurons[j]->deltavalues[k];
                udelta= alpha * (m_hiddenlayers[i]->neurons[j]->errorWeight / numberOfBatches)  + delta * momentum;
                m_hiddenlayers[i]->neurons[j]->weights[k]+=udelta;
                m_hiddenlayers[i]->neurons[j]->deltavalues[k]=udelta;
            ...
            m_hiddenlayers[i]->neurons[j]->wgain+=alpha * (m_hiddenlayers[i]->neurons[j]->errorGain / numberOfBatches) * m_hiddenlayers[i]->neurons[j]->gain;
    ...
    for(i=0;i<m_inputlayer.neuroncount;i++)
    {
        ...
        {
            delta=m_inputlayer.neurons[i]->deltavalues[j];
            udelta=alpha * (m_inputlayer.neurons[i]->errorWeight / numberOfBatches)  + delta * momentum;
            //update weights
            m_inputlayer.neurons[i]->weights[j]+=udelta;
            m_inputlayer.neurons[i]->deltavalues[j]=udelta;
        }
        //and update the gain weight
        m_inputlayer.neurons[i]->wgain+=alpha * (m_inputlayer.neurons[i]->errorGain / numberOfBatches) * m_inputlayer.neurons[i]->gain;
