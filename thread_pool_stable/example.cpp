...
struct data_wraper{
  bpnet * nnet;
  float **pattern,**desiredout;
  int *start;
  int pattern_size,counter,net_id;
...
void train_wrapper(void* arg)
{
  ...
  while(data1->counter >0){
    data1->nnet->batchTrain(data1->desiredout[i],data1->pattern[i]);
    i++;
...
int main(int argc, char *argv[])
{
  ...
  bpnet *netMatrix=new bpnet[NUM_THREADS];//Our neural network object
  ...
  int batch_per_thread=batch_size/NUM_THREADS;
    ...
    for(int i=0;i<1000000;i++)
    {
        ...
        {
          data2[j].nnet=&netMatrix[j];
          data2[j].pattern=pattern;
          data2[j].desiredout=desiredout;
          *data2[j].start=start_batch;
        ...
        while(flag)
        {
          if(pthread_mutex_trylock(&mutexBusy)==0)
          {
            ...
            for(int j=0;j<NUM_THREADS-1;j++){
              if(mutextable[j]==1){
                netMatrix[0].gatherErrors2(netMatrix,j);
                mutextable[j]=2;
              }
            }
