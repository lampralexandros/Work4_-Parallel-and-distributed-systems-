...
__global__ void cuda_layer_comulations(int neuroncount,int inputcount,int numberOfBatches,float alpha,float momentum,int gain,float *deltavalues,float *weights,float *wgain,float *errorWeight,float *errorGain)
{
  int i = threadIdx.x;
  int j = threadIdx.y;
  int index = i*(neuroncount) + j;
  if ( i < neuroncount && j < inputcount ){

    wgain[i]+= alpha * (errorGain[i] / numberOfBatches) * gain;
    errorGain[i]=0;
    errorWeight[i]=0;
      //now we proceed to update the weights of the neuron
          ...
          float udelta;
          //update the delta value
          udelta=alpha *(errorWeight[i] / numberOfBatches) + deltavalues[index] * momentum;
          //update the weight values
...
void bpnet::KernelapplyBatchCumulations(float alpha, float momentum)
{
    int i,j,k;
    int max_neurons,max_inputs;
    max_neurons=max_neuroncount();
    max_inputs=max_inputcount();



    // int *neuroncount_c,*inputcount_c,*numberOfBatches_c;//*gain_c;
    // // float *alpha_c,*momentum_c;
    // cudaMalloc((void **)&neuroncount_c, sizeof(int));
    // cudaMalloc((void **)&inputcount_c, sizeof(int));
    // cudaMalloc((void **)&numberOfBatches_c, sizeof(int));
    // cudaMalloc((void **)&gain_c, sizeof(int));
    // cudaMalloc((void **)&alpha_c, sizeof(float));
    // cudaMalloc((void **)&momentum_c, sizeof(float));
    ...
    float *errorWeight_C;
    float *errorGain_C;
    cudaMalloc((void **)&deltavalues_C,(max_neurons)*(max_inputs)*sizeof(float));
    cudaMalloc((void **)&weights_C,(max_neurons)*(max_inputs)*sizeof(float));
    cudaMalloc((void **)&wgain_C,(max_neurons)*sizeof(float));
    cudaMalloc((void **)&errorWeight_C,(max_neurons)*sizeof(float));
    cudaMalloc((void **)&errorGain_C,(max_neurons)*sizeof(float));


    float *deltavalues_D=new float[max_neurons*max_inputs];
    float *weights_D=new float[max_neurons*max_inputs];
    float *wgain_D=new float[max_neurons];
    float *errorWeight_D=new float[max_neurons];;
    float *errorGain_D=new float[max_neurons];;

    for(i=0;i<max_neurons;i++)
    {

        if(i<m_outputlayer.neuroncount){
          wgain_D[i]=m_outputlayer.neurons[i]->wgain;
          errorWeight_D[i]=m_outputlayer.neurons[i]->errorWeight;
          errorGain_D[i]=m_outputlayer.neurons[i]->errorGain;
        }else{
          wgain_D[i]=0;
          errorWeight_D[i]=0;
          errorGain_D[i]=0;
        ...
        {
          if(j<m_outputlayer.inputcount && i<m_outputlayer.neuroncount ){
            deltavalues_D[i*m_outputlayer.inputcount+j]=m_outputlayer.neurons[i]->deltavalues[j];
            weights_D[i*m_outputlayer.inputcount+j]=m_outputlayer.neurons[i]->weights[j];
          }else{
            deltavalues_D[i*m_outputlayer.inputcount+j]=0;
            weights_D[i*m_outputlayer.inputcount+j]=0;
          }
    ...
    // cudaMemcpy(neuroncount_c, m_outputlayer.neuroncount, sizeof(int), cudaMemcpyHostToDevice);
    // cudaMemcpy(inputcount_c, m_outputlayer.inputcount, sizeof(int), cudaMemcpyHostToDevice);
    // cudaMemcpy(numberOfBatches_c, m_outputlayer.numberOfBatches, sizeof(int), cudaMemcpyHostToDevice);
    // cudaMemcpy(gain_c,gain_d, sizeof(int), cudaMemcpyHostToDevice);
    // cudaMemcpy(alpha_c, alpha, sizeof(float), cudaMemcpyHostToDevice);
    // cudaMemcpy(momentum_c, monentum, sizeof(float), cudaMemcpyHostToDevice);

    cudaMemcpy(deltavalues_C, deltavalues_D, (max_neurons)*(max_inputs)*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(weights_C, weights_D, (max_neurons)*(max_inputs)*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(wgain_C, wgain_D, (max_neurons)*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(errorWeight_C, errorWeight_D, (max_neurons)*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(errorGain_C, errorGain_D, (max_neurons)*sizeof(float), cudaMemcpyHostToDevice);

    // dim3 dimBlock(32,32,1);
    // dim3 dimGrid(32,32,1);
    cuda_layer_comulations<<<1,m_outputlayer.neuroncount*m_outputlayer.inputcount>>>(m_outputlayer.neuroncount,m_outputlayer.inputcount,numberOfBatches,alpha,momentum,-1,deltavalues_C,weights_C,wgain_C,errorWeight_C,errorGain_C);
    ...
    cudaMemcpy(deltavalues_D, deltavalues_C, (max_neurons)*(max_inputs)*sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(weights_D, weights_C, (max_neurons)*(max_inputs)*sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(wgain_D, wgain_C, (max_neurons)*sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(errorWeight_D, errorWeight_C, (max_neurons)*sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(errorGain_D, errorGain_C, (max_neurons)*sizeof(float), cudaMemcpyDeviceToHost);
    ...
    for(i=0;i<m_outputlayer.neuroncount;i++)
    {
        m_outputlayer.neurons[i]->wgain=wgain_D[i];
        m_outputlayer.neurons[i]->errorWeight=errorWeight_D[i];
        m_outputlayer.neurons[i]->errorGain=errorGain_D[i];
        //now we proceed to update the weights of the neuron
        for(j=0;j<m_outputlayer.inputcount;j++)
        {
          m_outputlayer.neurons[i]->deltavalues[j]=deltavalues_D[i*m_outputlayer.inputcount+j];
          m_outputlayer.neurons[i]->weights[j]=weights_D[i*m_outputlayer.inputcount+j];
        }

    }


    for(k=(m_hiddenlayercount - 1);k>=0;k--)
    {

      for(i=0;i<max_neurons;i++)
      {
          if(i<m_hiddenlayers[k]->neuroncount){
            wgain_D[i]=m_hiddenlayers[k]->neurons[i]->wgain;
            errorWeight_D[i]=m_hiddenlayers[k]->neurons[i]->errorWeight;
            errorGain_D[i]=m_hiddenlayers[k]->neurons[i]->errorGain;
          }else{
            wgain_D[i]=0;
            errorWeight_D[i]=0;
            errorGain_D[i]=0;
          ...
          for(j=0;j<max_inputs;j++)
          {
            if(j<m_hiddenlayers[k]->inputcount && i<m_hiddenlayers[k]->neuroncount){
              deltavalues_D[i*m_hiddenlayers[k]->inputcount+j]=m_hiddenlayers[k]->neurons[i]->deltavalues[j];
              weights_D[i*m_hiddenlayers[k]->inputcount+j]=m_hiddenlayers[k]->neurons[i]->weights[j];
            }else{
              deltavalues_D[i*m_hiddenlayers[k]->inputcount+j]=0;
              weights_D[i*m_hiddenlayers[k]->inputcount+j]=0;
            }
      ...
      // dim3 dimBlock(32,32,1);
      // dim3 dimGrid(32,32,1);
      cuda_layer_comulations<<<1,m_hiddenlayers[k]->neuroncount*m_hiddenlayers[k]->inputcount>>>(m_hiddenlayers[k]->neuroncount,m_hiddenlayers[k]->inputcount,numberOfBatches,alpha,momentum,-1,deltavalues_C,weights_C,wgain_C,errorWeight_C,errorGain_C);
      ...
      for(i=0;i<m_hiddenlayers[k]->neuroncount;i++)
      {
          m_hiddenlayers[k]->neurons[i]->wgain=wgain_D[i];
          m_hiddenlayers[k]->neurons[i]->errorWeight=errorWeight_D[i];
          m_hiddenlayers[k]->neurons[i]->errorGain=errorGain_D[i];
          //now we proceed to update the weights of the neuron
          for(j=0;j<m_hiddenlayers[k]->inputcount;j++)
          {
            m_hiddenlayers[k]->neurons[i]->deltavalues[j]=deltavalues_D[i*m_hiddenlayers[k]->inputcount+j];
            m_hiddenlayers[k]->neurons[i]->weights[j]=weights_D[i*m_hiddenlayers[k]->inputcount+j];
          }
    ...
    for(i=0;i<max_neurons;i++)
    {
        ...
        if(i<m_inputlayer.neuroncount){
          wgain_D[i]=m_inputlayer.neurons[i]->wgain;
          errorWeight_D[i]=m_inputlayer.neurons[i]->errorWeight;
          errorGain_D[i]=m_inputlayer.neurons[i]->errorGain;
        }else{
          wgain_D[i]=0;
          errorWeight_D[i]=0;
          errorGain_D[i]=0;
        ...
        for(j=0;j<max_inputs;j++)
        {
          if(j<m_inputlayer.inputcount && i<m_inputlayer.neuroncount ){
            deltavalues_D[i*m_inputlayer.inputcount+j]=m_inputlayer.neurons[i]->deltavalues[j];
            weights_D[i*m_inputlayer.inputcount+j]=m_inputlayer.neurons[i]->weights[j];
          }else{
            deltavalues_D[i*m_inputlayer.inputcount+j]=0;
            weights_D[i*m_inputlayer.inputcount+j]=0;
          }
    ...
    // dim3 dimBlock(32,32,1);
    // dim3 dimGrid(32,32,1);
    cuda_layer_comulations<<<1,m_inputlayer.neuroncount*m_inputlayer.inputcount>>>(m_inputlayer.neuroncount,m_inputlayer.inputcount,numberOfBatches,alpha,momentum,-1,deltavalues_C,weights_C,wgain_C,errorWeight_C,errorGain_C);
    ...
    for(i=0;i<m_inputlayer.neuroncount;i++)
    {
        m_inputlayer.neurons[i]->wgain=wgain_D[i];
        m_inputlayer.neurons[i]->errorWeight=errorWeight_D[i];
        m_inputlayer.neurons[i]->errorGain=errorGain_D[i];
        //now we proceed to update the weights of the neuron
        for(j=0;j<m_inputlayer.inputcount;j++)
        {
          m_inputlayer.neurons[i]->deltavalues[j]=deltavalues_D[i*m_inputlayer.inputcount+j];
          m_inputlayer.neurons[i]->weights[j]=weights_D[i*m_inputlayer.inputcount+j];
        }
