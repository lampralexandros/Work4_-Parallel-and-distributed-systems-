...
#define MAX_NO_OF_WEIGHTS 9999//31
...
}; // changes to create N theNet;


void createNet( int, int *, int *, char *, double *, int , struct neuralNet);
...
void createNet( int noOfLayers, int *noOfNeurons, int *noOfInputs, char *axonFamilies, double *actFuncFlatnesses, int initWeights,struct neuralNet theNet  ) {
...
void feedNetInputs(double *inputs, struct neuralNet theNet) {
     int i;
...
void updateNetOutput(struct neuralNet theNet ) {
...
void applyBatchCumulations( double learningRate, double momentumRate, struct neuralNet theNet) {
...
double *getOutputs(struct neuralNet theNet) {
...
int main() {
    ...
    // creating the newNet pointer to many nets
	static struct  neuralNet *manyNets;
	// random number
	int howManyNet=1;
  ...
  createNet(noOfLayers, noOfNeurons, noOfInputs, axonFamilies, actFuncFlatnesses, 1, manyNets[0]);
    ...
    for(i = 0; i < N; i++) {
        ...
        feedNetInputs(inputs, manyNets[0]);
        updateNetOutput(manyNets[0]);
    ...
    double timePassed = (double)((endwtime.tv_usec - startwtime.tv_usec)
    ...
    for(i = 0; i < 50; i++) {
        ...
        feedNetInputs(inputs,manyNets[0]);
        updateNetOutput(manyNets[0]);
        outputs = getOutputs(manyNets[0]);
        average_error+=fabs((double)sin(tempTotal)-(double)outputs[0])+fabs((double)cos(tempTotal)-(double)outputs[1]);
        countTest+=2;
        //printf( "%f \t %f \t %f \t %f \n", sin(tempTotal), outputs[0], cos(tempTotal), outputs[1]);
    }
    average_error=average_error/countTest;
